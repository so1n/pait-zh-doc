...
!!! note
    ...
    - 2.这两个插件都要单独根据不同的Web框架进行兼容，所以请以`from pait.app.{web framework name}.plugin.{plugin name} import xxx`的形式来引入对应的插件。
...
@pait(response_model_list=[UserSuccessRespModel3], plugin_list=[AsyncCheckJsonRespPlugin.build()])
async def demo(
...
然后在41行中引入了一个名为`AsyncCheckJsonRespPlugin`的插件，该插件会在启动的时候检查路由函数定义的返回类型与`UserSuccessRespModel3.response_data`是否一致，不一致则会报错。
...
@pait(
    ...
    plugin_list=[AsyncAutoCompleteJsonRespPlugin.build()]
)
async def demo(
    uid: int = field.Query.i(description="user id", gt=10, lt=1000),
    email: Optional[str] = field.Query.i(default="example@xxx.com", description="user email"),
...
```
通过响应结果可以发现，对于第一个响应结果的`age`值为调用命令时填写的18，而第二个响应结果中本来是没有`age`字段的，该字段值是由插件`AsyncAutoCompleteJsonRespPlugin`根据`age`的类型自动填上默认值0。

`AsyncAutoCompleteJsonRespPlugin`自动补全的原理是从`response_model_list`中选出开发者填写的第一个`ResponseModel`，比如代码中的例子就是`UserSuccessRespModel3`，
