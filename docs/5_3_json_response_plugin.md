...
```
## 缓存插件
缓存插件能缓存除流式响应意外的响应对象，同时也能根据函数签名来自动生成缓存的Key, 所以缓存插件属于后置插件，这样才能读取到函数签名的参数。 缓存插件示例代码如下：
```Python
import time
...
def create_app() -> Starlette:
    ...
    # 延后绑定Redis，这样可以在不同的实例使用不同的Redis实例
...
1652714661.9645622  
```
上面是缓存插件的简单使用方法，目前还支持其它的参数，如下：
- redis: 指定插件可以使用的Redis实例，建议通过`CacheResponsePlugin.set_redis_to_app`方法指定Redis实例。
- name: 指定路由函数的缓存Key，如果为空，则以函数名为Key。
- enable_cache_name_merge_param: 如果为True，则会
- cache_time: 缓存时间
- timeout: 锁的最长时间，多个请求会争夺一个锁。
- sleep: 当锁处于阻塞模式并且另一个客户端当前持有锁时，每次循环迭代的休眠时间。
